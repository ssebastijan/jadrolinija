...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `promjeniLiniju` BEFORE UPDATE ON `linija` FOR EACH ROW BEGIN
        IF TIME_TO_SEC(NEW.satDolaska) <= TIME_TO_SEC(NEW.satOdlaska) THEN
          signal sqlstate '45001' set message_text = 'Vrijeme dolaska mora biti veće od vremena odlaska', MYSQL_ERRNO = 1001;
        END IF;
        IF NEW.sifraOdlaznogPristanista = NEW.sifraDolaznogPristanista THEN
...
/*!50003 SET sql_mode              = 'NO_AUTO_VALUE_ON_ZERO' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `updatePistaniste` BEFORE UPDATE ON `pristaniste` FOR EACH ROW BEGIN
        if new.sifPristanista = new.sifraNadredenogPristanista THEN
          signal sqlstate '45004' set message_text = 'Nadredjeno pristanište ne smije biti isto kao i ovo pristanište', MYSQL_ERRNO = 1004;
        END IF;
  END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
